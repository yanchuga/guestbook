...
{
$result = mysql_query ("UPDATE records SET name='$name', email='$email', date='$date', url='$url', text='$text' WHERE id='$id'");

if ($result) {echo "<p>ВАШ ПОСТ УСПЕШНО ИЗМЕНЕН! ВЫ МОЖЕТЕ <a href='../index.php'>ПОСМОТРЕТЬ ПОСТ</a></p>";}
else {echo "<p>ВАШ ПОСТ НЕ ИЗМЕНЁН!</p>";}
}		 
else 
{
echo "<p>ВАШ ПОСТ НЕ ИЗМЕНЁН! ОБНОВИТЕ СТРАНИЦУ!</p>";
}
?>
      </div>
      </div>
